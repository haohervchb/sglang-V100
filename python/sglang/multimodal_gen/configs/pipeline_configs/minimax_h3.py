...
class MiniMaxH3PipelineConfig(PipelineConfig):
    ...
    # Volta has FP16 Tensor Cores but no BF16 execution support. Keep the
    # V100 fork's complete H3 path in FP16 unless an individual operation
    # explicitly requires FP32 accumulation.
    dit_precision: str = "fp16"
    vae_precision: str = "fp16"
    vae_decode_precision: str = "fp16"
    audio_vae_precision: str = "fp16"
    ...
    def get_model_deployment_config(self) -> ModelDeploymentConfig:
        # This fork's auto-tuner already treats performance_mode=speed as a
        # resident deployment. TP4 is explicitly selected by the launch
        # command, so no FSDP/CFG auto-policy is needed here.
    ...
    def validate_quality_deployment(self, server_args) -> None:
        ...
        actual = {
            ...
            "enable_breakable_cuda_graph": getattr(
            ...
            "regional_compile": getattr(server_args, "regional_compile", False),
    ...
    def validate_server_args(self, server_args) -> None:
        ...
        capability = current_platform.get_device_capability()
        capability_int = capability.to_int() if capability is not None else None
        if current_platform.is_cuda() and capability_int == 70:
            precisions = {
                "dit": self.dit_precision,
                "text_encoder": self.text_encoder_precisions[0],
                "video_vae": self.vae_precision,
                "audio_vae": self.audio_vae_precision,
            }
            non_fp16 = {name: value for name, value in precisions.items() if value != "fp16"}
            if non_fp16:
                raise ValueError(
                    "MiniMax-H3 on V100 requires FP16 component precisions; "
                    f"got {non_fp16}."
                )
        attention_backend = self._server_arg_value(server_args.attention_backend)
