...
class MiniMaxH3Qwen3VLEncoder(TextEncoder):
    ...
    def device(self) -> torch.device:
        # With CPU offload, parameters describe their storage device while the
        # gathered forward still runs on this rank's accelerator.
        ...
        *,
        ...
        if pixel_values is not None:
            call_kwargs["pixel_values"] = pixel_values.to(
                self.device, self.compute_dtype
            )
        ...
        hidden = self.model(**call_kwargs).last_hidden_state[0].to(self.compute_dtype)
