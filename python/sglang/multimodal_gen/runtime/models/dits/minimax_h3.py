...
def _reduced_precision_dtype() -> torch.dtype:
    """Return the loader-selected DiT dtype, retaining BF16 test defaults."""
...
def _copy_grouped_qkv_tp_shard(
    ...
    *,
    ...
    if (
        ...
        or param.dtype not in (torch.float16, torch.bfloat16)
        or loaded_weight.dtype != param.dtype
...
def _norm(size: int, *, eps: float, dtype: torch.dtype | None = None) -> nn.RMSNorm:
    ...
    return nn.RMSNorm(size, eps=eps, dtype=dtype or _reduced_precision_dtype())
...
def _apply_rope_qk(
    ...
    half = selected_cache.shape[-1] // 2
    cos_half, sin_half = selected_cache.split(half, dim=-1)
...
class MiniMaxH3Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.q_norm = _norm(
            arch.attention_head_dim, eps=arch.qk_norm_eps, dtype=compute_dtype
        )
        self.k_norm = _norm(
            arch.attention_head_dim, eps=arch.qk_norm_eps, dtype=compute_dtype
        )
...
class MiniMaxH3TokenRefinerBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.norm1 = _norm(arch.hidden_size, eps=arch.norm_eps, dtype=compute_dtype)
        self.norm2 = _norm(arch.hidden_size, eps=arch.norm_eps, dtype=compute_dtype)
    ...
    def __init__(
        ...
        *,
        ...
        self.final_norm = _norm(
            arch.hidden_size, eps=arch.final_norm_eps, dtype=compute_dtype
        )
    ...
    def forward(
        ...
        *,
        ...
        h = _modulate_scale_shift(
            h, shift_msa, scale_msa, combined_indices, dtype=self.compute_dtype
        )
        h = self.attn(
            h,
        ...
        x = _modulate_gate(
            residual, gate_msa, h, combined_indices, dtype=self.compute_dtype
        )

        residual = x
        h = self.norm2(x)
        h = _modulate_scale_shift(
            h, shift_mlp, scale_mlp, combined_indices, dtype=self.compute_dtype
        )
        h = self.mlp(h)
        return _modulate_gate(
            residual, gate_mlp, h, combined_indices, dtype=self.compute_dtype
        )
...
class MiniMaxH3FinalLayer(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        h = self.norm(x)
        h = _modulate_scale_shift(
            h, shift, scale, inverse_indices, dtype=self.compute_dtype
        )
...
class MiniMaxH3DiTModel(BaseDiT, LayerwiseOffloadableModuleMixin):
    ...
    def __init__(
        ...
        self.arch = arch
        self.compute_dtype = _reduced_precision_dtype()
        self.hidden_size = arch.hidden_size
    ...
    def refine_prompt_embeds(
        ...
        *,
        ...
        text_rows = prompt_embeds[:text_len].to(device=device, dtype=self.compute_dtype)
    ...
    def build_rope_cache(
        ...
        *,
        ...
        return (
            _rope_cos_sin_cache(rope_freqs, dtype=self.compute_dtype),
            torch.arange(
                local_seq_len,
                device=device,
                dtype=torch.long,
        ...
        *,
        ...
        if trusted_layout:
            ...
            write_rows(
                0,
                text_row_ids,
                text_embed.index_select(0, text_source_ids).to(self.compute_dtype),
            )
        ...
        if img_row_ids.numel():
            ...
            write_rows(
                0,
                img_row_ids,
                video_embed.to(self.compute_dtype),
            )
        ...
        if audio_row_ids.numel():
            ...
            write_rows(
                0,
                audio_row_ids,
                audio_embed.to(self.compute_dtype),
            )
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        adaln_input = nn.functional.silu(t_emb).to(self.compute_dtype)
