...
class DFlashWorker:
    ...
    def _prepare_for_speculative_decoding(
        ...
        if batch.seq_lens_cpu is not None:
            # The scheduler already maintains exact committed lengths on CPU.
            # Reuse them instead of synchronizing draft_prefix_lens from CUDA
            # before every draft graph replay. The old D2H copy serialized the
            # target commit, draft KV materialization, and the next draft launch.
            if self.use_compact_draft_cache:
                seq_lens_cpu.copy_(
                    self._compute_compact_draft_seq_lens(batch.seq_lens_cpu)
                )
            else:
        ...
        try:
            ...
            # Use the host mirror for planning metadata.  Reading the equivalent
            # CUDA reduction with .item() introduced a second stream sync here.
