...
variants = json.loads((p / "moe_unroll_variants.json").read_text())
mods = {tuple(v): load(f"moe_g{v[0]}_r{v[1]}_t{v[2]}") for v in variants}
for m in [4, 2, 1]:
    ...
    for (gu, ru, nt), (mod, fs) in mods.items():
        split, down = 40, 5
        ...
        with (p / "moe_unroll_tuning.jsonl").open("a") as f:
            f.write(json.dumps(row) + "\n")
    del w13, w2, s13, s2, ref
(p / "moe_unroll_tuning.done").write_text("done")
