...
def _expand_qsa_block_indices_kernel(
    ...
    SORT_BLOCKS: tl.constexpr = False,
):
    row = tl.program_id(0)
    ...
    if SORT_BLOCKS:
        ...
        selected = tl.load(block_indices + row * block_stride + block_cols)
        # Radix selection emits a set in atomic collection order. Canonical
        # logical order makes the FP16 attention reductions repeatable for
        # the same selected set and groups nearby cache rows together.
        ordered = tl.sort(tl.where(selected >= 0, selected, 0x7FFFFFFF))
        blocks = tl.gather(ordered, tl.minimum(source_block_cols, BLOCK_TOPK - 1), 0)
        blocks = tl.where(blocks == 0x7FFFFFFF, -1, blocks)
    else:
        blocks = tl.load(
...
def triton_expand_qsa_block_indices(
    ...
    _expand_qsa_block_indices_kernel[(rows,)](
        ...
        SORT_BLOCKS=(
            os.environ.get("SGLANG_SM70_MTP_QSA_ORDER", "0") == "1"
            and block_topk == 512
            and compress_ratio == 4
            and torch.cuda.get_device_capability(block_indices.device) == (7, 0)
        ),
        num_warps=8,
    )
    return output
